...
def _build_calendar_context(
    ...
    # grouped separately from one-off events.  Sort by start
    # timestamp string (ISO 8601 sorts lexicographically) so the
    # earliest appointment is always shown first.
    sorted_events = sorted(
        raw_events, key=lambda e: str(e.get("start", ""))
    )
